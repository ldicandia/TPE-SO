...
void place_players(GameState *state) {
  for (int i = 0; i < state->num_players; i++) {
    ...
    snprintf(state->players[i].name, sizeof(state->players[i].name), "Player%d",
             i + 1);
  }
}
...
size_t get_game_state_size() { return sizeof(GameState); }

void set_width(GameState *state, int width) { state->width = width; }

void set_height(GameState *state, int height) { state->height = height; }
...
bool is_game_over(const GameState *state) { return state->game_over; }
...
int get_num_players(const GameState *state) { return state->num_players; }
...
int get_width(const GameState *state) { return state->width; }

int get_height(const GameState *state) { return state->height; }
...
int get_players_valid_moves(GameState *state, int player_idx) {
  return state->players[player_idx].valid_moves;
}

int get_players_invalid_moves(GameState *state, int player_idx) {
  return state->players[player_idx].invalid_moves;
}
